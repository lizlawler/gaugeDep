...
# # 
# data_num <- 2
# data <- RcppSimdJson::fload(sprintf("data/gauss/high_%s.json", data_num))
# new_w <- mix_dens_rng(50000, test_ang_mix[data_num,])
# 
# r <- data$R
# w <- data$W
# x <- r * w
# y <- r * (1-w)
# gw_fitted <- gauss_gauge(w, 1-w, test_radial$dep[data_num])
...
# gw_pseudo <- gauss_gauge(pseudo_pred$w1_pseudo, pseudo_pred$w2_pseudo, test_radial$dep[data_num])
# poss_k <- pseudo_pred$r_pseudo * gw_pseudo / ctau_fitted
# (k <- max(round(min(poss_k), 1) - 0.5, 1))
# 
# N <- 50000
# gw <- gauss_gauge(new_w, 1 - new_w, test_radial$dep[data_num])
# r0w <- qgamma(0.95, test_radial$alpha[data_num], gw)
# wts <- imp_weights(k = k, w_star = new_w, r0w_star = r0w, post_radial_alpha = test_radial$alpha[data_num], post_rate = gw)
# resample_idx <- sample(1:N, size = N, replace = TRUE, prob = wts)
# new_wstar <- new_w[resample_idx]
# gw <- gw[resample_idx]
# r0w <- k * r0w[resample_idx]
# 
# rstar <- qgamma(1 - runif(N) * pgamma(r0w, shape = test_radial$alpha[data_num],
#                                       rate = gw, lower.tail = F),
#                 shape = test_radial$alpha[data_num], rate = gw)
# 
# xy <- cbind(rstar * new_wstar, rstar * (1 - new_wstar)) |> as_tibble() |> rename(x = V1, y = V2)
# 
