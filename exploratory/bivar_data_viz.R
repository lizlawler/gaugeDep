# =============================================================================
# Visualises the bivariate dependence structure for simulated data under each
# of the three data-generating models (Gaussian, logistic, Husler-Reiss) at
# three dependence levels (low, mid, high). Used to produce the data illustration
# figures. Also shows the radial/angular decomposition and threshold exceedances.
#
# Inputs:    data/{dep_type}/{dep_level}_{i}.json
# Outputs:   figures/copulas/ (PNG files)
# =============================================================================
...
low_dep_gauss_points <- low_dep_gauss |>
  as_tibble() |>
  mutate(
    q1 = quantile(low_dep_gauss[, 1], 0.95),
...
low_dep_gauss_stan <- low_dep_gauss_points |>
  filter(high == 1) |>
  select(r, w)
stan_low_dep_gauss <- list(
  R = low_dep_gauss_stan$r,
...
low_dep_gauss_plot <- low_dep_gauss_points |>
  ...
  xlab("y1") +
  ylab("y2") +
  ggtitle(expression(paste("Gaussian copula, ", rho, "=0.1,", " expo margins")))
...
mid_dep_gauss_points <- mid_dep_gauss |>
  as_tibble() |>
  mutate(
    qy1 = quantile(mid_dep_gauss[, 1], 0.95), qy2 = quantile(mid_dep_gauss[, 1], 0.95),
...
mid_dep_gauss_stan <- mid_dep_gauss_points |>
  filter(high == 1) |>
  select(r, w)
stan_mid_dep_gauss <- list(
  R = mid_dep_gauss_stan$r,
...
mid_dep_gauss_plot <- mid_dep_gauss_points |>
  ...
  xlab("y1") +
  ylab("y2") +
  ggtitle(expression(paste("Gaussian copula, ", rho, "=0.5,", " expo margins")))
...
high_dep_gauss_points <- high_dep_gauss |>
  as_tibble() |>
  mutate(
    qy1 = quantile(high_dep_gauss[, 1], 0.95), qy2 = quantile(high_dep_gauss[, 1], 0.95),
    high = case_when(y1 >= qy1 | y2 >= qy2 ~ 1,
      .default = 0
    ),
    high = as.factor(high),
...
high_dep_gauss_stan <- high_dep_gauss_points |>
  filter(high == 1) |>
  select(r, w)
stan_high_dep_gauss <- list(
  R = high_dep_gauss_stan$r,
...
high_dep_gauss_plot <- high_dep_gauss_points |>
  ...
  xlab("y1") +
  ylab("y2") +
  ggtitle(expression(paste("Gaussian copula, ", rho, "=0.9,", " expo margins")))
...
high_dep_log_points <- high_dep_log |>
  as_tibble() |>
  mutate(
    qy1 = quantile(high_dep_log[, 1], 0.95), qy2 = quantile(high_dep_log[, 1], 0.95),
    high = case_when(y1 >= qy1 | y2 >= qy2 ~ 1,
      .default = 0
    ),
    high = as.factor(high),
...
high_dep_log_stan <- high_dep_log_points |>
  filter(high == 1) |>
  select(r, w)
stan_high_dep_log <- list(
  R = high_dep_log_stan$r,
...
high_dep_log_plot <- high_dep_log_points |>
  ...
  xlab("y1") +
  ylab("y2") +
  ggtitle(expression(paste("Logistic copula, ", "r", "=0.1,", " expo margins")))
...
mid_dep_log_points <- mid_dep_log |>
  as_tibble() |>
  mutate(
    qy1 = quantile(mid_dep_log[, 1], 0.95), qy2 = quantile(mid_dep_log[, 1], 0.95),
    high = case_when(y1 >= qy1 | y2 >= qy2 ~ 1,
      .default = 0
    ),
    high = as.factor(high),
...
mid_dep_log_stan <- mid_dep_log_points |>
  filter(high == 1) |>
  select(r, w)
stan_mid_dep_log <- list(
  R = mid_dep_log_stan$r,
...
column_label_1 <- wrap_elements(panel = ggpubr::text_grob(
  label = "Pseudo-polar coordinates",
  face = "bold",
  size = 20
))
column_label_2 <- wrap_elements(panel = ggpubr::text_grob(
  label = "Euclidean coordinates",
  face = "bold",
  size = 20
))
...
low_dep_log_points <- low_dep_log |>
  as_tibble() |>
  mutate(
    qy1 = quantile(low_dep_log[, 1], 0.95), qy2 = quantile(low_dep_log[, 1], 0.95),
    high = case_when(y1 >= qy1 | y2 >= qy2 ~ 1,
      .default = 0
    ),
    high = as.factor(high),
...
low_dep_log_stan <- low_dep_log_points |>
  filter(high == 1) |>
  select(r, w)
stan_low_dep_log <- list(
  R = low_dep_log_stan$r,
...
low_dep_log_plot <- low_dep_log_points |>
  ...
  xlab("y1") +
  ylab("y2") +
  ggtitle(expression(paste("Logistic copula, ", "r", "=0.9,", " expo margins")))
...
low_dep_invlog_points <- low_dep_invlog |>
  as_tibble() |>
  rename(y1 = V1, y2 = V2) |>
  mutate(
    qy1 = quantile(low_dep_invlog[, 1], 0.95), qy2 = quantile(low_dep_invlog[, 1], 0.95),
    high = case_when(y1 >= qy1 | y2 >= qy2 ~ 1,
      .default = 0
    ),
    high = as.factor(high),
...
low_dep_invlog_stan <- low_dep_invlog_points |>
  filter(high == 1) |>
  select(r, w)
stan_low_dep_invlog <- list(
  R = low_dep_invlog_stan$r,
...
low_dep_invlog_plot <- low_dep_invlog_points |>
  ...
  xlab("y1") +
  ylab("y2") +
  ggtitle(expression(paste("Inverse logistic copula, ", "r", "=0.9,", " expo margins")))
...
mid_dep_invlog_points <- mid_dep_invlog |>
  as_tibble() |>
  rename(y1 = V1, y2 = V2) |>
  mutate(
    qy1 = quantile(mid_dep_invlog[, 1], 0.95), qy2 = quantile(mid_dep_invlog[, 1], 0.95),
    high = case_when(y1 >= qy1 | y2 >= qy2 ~ 1,
      .default = 0
    ),
    high = as.factor(high),
...
mid_dep_invlog_stan <- mid_dep_invlog_points |>
  filter(high == 1) |>
  select(r, w)
stan_mid_dep_invlog <- list(
  R = mid_dep_invlog_stan$r,
...
mid_dep_invlog_plot <- mid_dep_invlog_points |>
  ...
  xlab("y1") +
  ylab("y2") +
  ggtitle(expression(paste("Inverse logistic copula, ", "r", "=0.5,", " expo margins")))
...
high_dep_invlog_points <- high_dep_invlog |>
  as_tibble() |>
  rename(y1 = V1, y2 = V2) |>
  mutate(
    qy1 = quantile(high_dep_invlog[, 1], 0.95), qy2 = quantile(high_dep_invlog[, 1], 0.95),
    high = case_when(y1 >= qy1 | y2 >= qy2 ~ 1,
      .default = 0
    ),
    high = as.factor(high),
...
high_dep_invlog_stan <- high_dep_invlog_points |>
  filter(high == 1) |>
  select(r, w)
stan_high_dep_invlog <- list(
  R = high_dep_invlog_stan$r,
...
high_dep_invlog_plot <- high_dep_invlog_points |>
  ...
  xlab("y1") +
  ylab("y2") +
  ggtitle(expression(paste("Inverse logistic copula, ", "r", "=0.1,", " expo margins")))
